...
inline void timer_init(string arg="default"){
    timeval ts;
    gettimeofday(&ts,NULL);
    __head_h_start[arg]=ts;
}
inline int64 timer_elapse(string arg="default"){ // unit ms
    struct timeval now;
...
inline string __n_variable(string t, int n){ t=t+','; int i=0; if(n) for(; i<SIZE(t)&&n; i++) if(t[i]==',') n--; n=i; for(;t[i]!=',';i++); t=t.substr(n, i-n); trim(t);if(t[0]=='"') return ""; return t+"="; }
...
template<class T>
inline string toStr(T t){ stringstream ss; ss<<t; return ss.str(); }
inline string strcat2(const string a, const string b){return (a+b); }

inline string currentTimestampStr() {
...
inline void ExitMessage(string msg)
{
    cout << msg << endl;
    exit(1);
}
